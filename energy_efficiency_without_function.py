# Initialize an empty dictionary to store the results
    ...
    for w_i in range(0, len(w_direction)):
        ...
        for t_k in range(0, N_turbines):
            ...
            p_ideal_k, C_k = get_power_and_thrust(w_speed[w_i], p_curve)

            other_turbines = [t for t in range(0, N_turbines) if t != t_k]
    ...
    term1 = sum([ sum(results[w_direction[w]]['p_generated'][t] * w_frequency[w] for t in range(0, N_turbines)) for w in range(0,len(w_direction))])

    term2 = sum([ N_turbines * results[w_direction[w]]['p_generated_without_wake'][0] * w_frequency[w] for w in range(0,len(w_direction))])
